#!/usr/bin/env python3
"""Run frozen Gemma 2 9B inference on the stored Vosk FLEURS transcripts."""
...
from benchmark.asr_llm_error_propagation.llm.run_gemma2_9b_asr_path import (
    OUTPUT_COLUMNS,
    append_row,
    completed_filenames,
    load_asr_items,
...
DEFAULT_INPUT = PROJECT_DIR / "analysis/inputs/vosk_fleurs_qa_eval_input_v1.csv"
DEFAULT_ORACLE = SCRIPT_DIR / "gemma2_9b_oracle_predictions_v1.csv"
DEFAULT_MODEL = REPOSITORY_DIR / "models/llm/gemma-2-9b-it-4bit"
DEFAULT_OUTPUT = SCRIPT_DIR / "gemma2_9b_vosk_path_predictions_v1.csv"
DEFAULT_METADATA = SCRIPT_DIR / "gemma2_9b_vosk_path_predictions_v1.metadata.json"
...
def main() -> int:
    ...
    if unknown := recorded - expected_filenames:
        raise ValueError(f"Vosk-path checkpoint contains {len(unknown)} unknown files")
    ...
    if len(recorded) == len(items):
        print(f"Vosk-path checkpoint is already complete: {args.output.resolve()}")
        return 0
    ...
    print(f"Gemma 2 9B Vosk path | CUDA | remaining: {len(remaining)}/769")
    ...
    for index, item in enumerate(remaining, start=1):
        ...
        write_header = False
        print(f"[{len(recorded) + index:03d}/769] {item['file_name']}: {status}", flush=True)

    if sha256_file(args.input) != input_hash:
        raise RuntimeError("Frozen Vosk input changed during inference")
    if sha256_file(args.oracle) != oracle_hash:
        raise RuntimeError("Frozen Oracle predictions changed during inference")
    with args.output.open("r", encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != 769 or len({row["file_name"] for row in rows}) != 769:
        raise RuntimeError("Vosk-path checkpoint is incomplete or duplicated")

    successful = [row for row in rows if row["status"] == "ok"]
    metadata = {
        "stage": "gemma2_9b_vosk_path_v1",
        ...
        "only_condition_difference_from_oracle": (
            "context=Vosk asr_normalized instead of context=reference_normalized"
        ),
    ...
    with args.metadata.open("w", encoding="utf-8") as handle:
        ...
        handle.write("\n")
    print(f"Frozen Vosk-path predictions: {args.output.resolve()}")
    print(f"Vosk-path metadata: {args.metadata.resolve()}")
    return 0
